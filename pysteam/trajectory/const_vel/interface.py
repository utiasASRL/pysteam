...
class Interface(TrajInterface):
    ...
    def add_pose_prior(self, time: Time, T_k0: Transformation, cov: np.ndarray) -> None:
        ...
        # create cost term
        self._pose_prior_factor = WeightedLeastSquareCostTerm(
            error_func, noise_model, loss_func
        )

    def add_velocity_prior(
        self, time: Time, w_0k_ink: np.ndarray, cov: np.ndarray
    ) -> None:
        ...
        # create cost term
        self._velocity_prior_factor = WeightedLeastSquareCostTerm(
            error_func, noise_model, loss_func
        )
    ...
    def get_prior_cost_terms(self) -> List[CostTerm]:
        ...
        for t in range(1, len(self._ordered_nsecs)):
            ...
            if (
                knot1.pose.active
                or knot1.velocity.active
                or knot2.pose.active
                or knot2.velocity.active
            ):
                ...
                cost_term = WeightedLeastSquareCostTerm(
                    error_func, noise_model, loss_func
                )
    ...
    def get_pose_interpolator(self, time: Time):
        ...
        if idx == 0 or idx == len(self._ordered_nsecs):
            ...
            if idx == 0:
                ...
                T_t_k_eval = PoseExtrapolator(
                    start_knot.velocity, time - start_knot.time
                )
    ...
    def get_covariance(self, cov: Covariance, time: Time):
        ...
        if idx < len(self._ordered_nsecs) and self._ordered_nsecs[idx] == time.nanosecs:
            ...
            assert isinstance(T_k0, se3ev.SE3StateVar) and isinstance(
                w_0k_ink, vspaceev.VSpaceStateVar
            )
        ...
        # extrapolate
            ...
            knot = (
                self._knots[self._ordered_nsecs[0]]
                if idx == 0
                else self._knots[self._ordered_nsecs[-1]]
            )

            T_q0_var = se3ev.SE3StateVar(self.get_pose_interpolator(time).evaluate())
            w_0q_inq_var = vspaceev.VSpaceStateVar(
                self.get_velocity_interpolator(time).evaluate()
            )
            ...
            if idx == 0:  # extrapolate before first knot
                loss_func = L2LossFunc()
                noise_model = StaticNoiseModel(
                    getQinv((knot.time - knotq.time).seconds, self._qcd), "information"
                )
                ...
                loss_func = L2LossFunc()
                noise_model = StaticNoiseModel(
                    getQinv((knotq.time - knot.time).seconds, self._qcd), "information"
                )
            ...
            loss_func = L2LossFunc()
            noise_model = StaticNoiseModel(
                cov.query([knot.pose, knot.velocity]), "covariance"
            )
            error_func = state_error(
                se3ev.se3_error(knot.pose, knot.pose.value),
            ...
            indices_slices = [
                state_vector.get_state_indices(var.key)
                for var in [knotq.pose, knotq.velocity]
            ]
        ...
        # interpolate
            ...
            loss_func = L2LossFunc()
            noise_model = StaticNoiseModel(
                getQinv((knot2.time - knot1.time).seconds, self._qcd), "information"
            )
            error_func = PriorFactor(knot1, knot2)
            cost_term = WeightedLeastSquareCostTerm(error_func, noise_model, loss_func)
            cost_term.build_gauss_newton_terms(state_vector, A, b)
            ...
            loss_func = L2LossFunc()
            noise_model = StaticNoiseModel(
                getQinv((knotq.time - knot1.time).seconds, self._qcd), "information"
            )
            ...
            loss_func = L2LossFunc()
            noise_model = StaticNoiseModel(
                getQinv((knot2.time - knotq.time).seconds, self._qcd), "information"
            )
            ...
            loss_func = L2LossFunc()
            noise_model = StaticNoiseModel(
                cov.query([knot1.pose, knot1.velocity, knot2.pose, knot2.velocity]),
                "covariance",
            )
            knot1_error = state_error(
                se3ev.se3_error(knot1.pose, knot1.pose.value),
                vspaceev.vspace_error(knot1.velocity, knot1.velocity.value),
            )
            knot2_error = state_error(
                se3ev.se3_error(knot2.pose, knot2.pose.value),
