...
class VelocityInterpolator(Evaluable):
    def __init__(
        self, time: Time, k1: Variable, k2: Variable, k3: Variable, k4: Variable
    ) -> None:
        super().__init__()
        ...
        ratio = (t - b) / (bp1 - b)
        u = np.array([[1.0, ratio, ratio**2, ratio**3]]).T

        self._w = (B.T @ u).flatten()

    @property
    def active(self) -> bool:
        return (self._k1.c.active or self._k2.c.active) or (
            self._k3.c.active or self._k4.c.active
        )

    @property
    def related_var_keys(self) -> set:
        return (
            self._k1.c.related_var_keys
            | self._k2.c.related_var_keys
            | self._k3.c.related_var_keys
            | self._k4.c.related_var_keys
        )
