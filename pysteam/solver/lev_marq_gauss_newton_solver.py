...
class LevMarqGaussNewtonSolver(GaussNewtonSolver):
    ...
    def linearize_solve_and_update(self):
        ...
        while num_backtrack < self._parameters["max_shrink_steps"]:
            ...
            if (
                decomp_success
                and actual_to_predicted_ratio > self._parameters["ratio_threshold"]
            ):
                self.accept_proposed_state()
                self._diag_coeff = max(
                    self._diag_coeff * self._parameters["shrink_coeff"], 1e-7
                )
                ...
                self._diag_coeff = min(
                    self._diag_coeff * self._parameters["grow_coeff"], 1e7
                )
                num_tr_decreases += 1
        ...
        if self._parameters["verbose"]:
            print(
                f"Iteration: {self._curr_iteration:4}  -  Cost: {new_cost:10.4f}  -  TR Shrink: {num_tr_decreases:6.3f}  -  AvP Ratio: {actual_to_predicted_ratio:6.3f}"
            )
