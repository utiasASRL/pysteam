...
class Solver(abc.ABC):
    ...
    def iterate(self) -> None:
        ...
        if not step_success and np.abs(grad_norm) < 1e-6:
            ...
            np.abs(self._prev_cost - self._curr_cost)
            <= self._parameters["absolute_cost_change_threshold"]
        ):
            self._term = "CONVERGED ABSOLUTE CHANGE"
            self._solver_converged = True
        elif (
            np.abs(self._prev_cost - self._curr_cost) / self._prev_cost
        ) <= self._parameters["relative_cost_change_threshold"]:
