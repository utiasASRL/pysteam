...
class DoglegGaussNewtonSolver(GaussNewtonSolver):
    ...
    def linearize_solve_and_update(self):
        ...
        while num_backtrack < self._parameters["max_shrink_steps"]:
            if gauss_newton_norm <= self._trust_region_size:
                ...
                perturbation = (
                    self._trust_region_size / grad_descent_norm
                ) * grad_descent_step
                ...
                interp_const = (
                    -gd_dot_prod_gd_to_gn
                    + np.sqrt(
                        gd_dot_prod_gd_to_gn**2
                        + (self._trust_region_size**2 - grad_descent_norm**2)
                        * gd_to_gn_sqr_norm
                    )
                ) / gd_to_gn_sqr_norm
            ...
            if actual_to_predicted_ratio > self._parameters["ratio_threshold_shrink"]:
                ...
                if actual_to_predicted_ratio > self._parameters["ratio_threshold_grow"]:
                    self._trust_region_size = max(
                        self._trust_region_size,
        ...
        if self._parameters["verbose"]:
            print(
                f"Iteration: {self._curr_iteration:4}  -  Cost: {new_cost:10.4f}  -  TR Shrink: {num_tr_decreases:6.3f}  -  AvP Ratio: {actual_to_predicted_ratio:6.3f}  -  Dogleg Segment: {dogleg_segment:15}"
            )
    ...
    def predict_reduction(
        self, A: np.ndarray, b: np.ndarray, step: np.ndarray
    ) -> float:
