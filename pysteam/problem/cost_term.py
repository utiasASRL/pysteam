...
class CostTerm(abc.ABC):
    ...
    @abc.abstractmethod
    def build_gauss_newton_terms(
        self, state_vector: StateVector, A: np.ndarray, b: np.ndarray
    ):
...
class WeightedLeastSquareCostTerm(CostTerm):
    ...
    def eval_weighted_and_whitened(self) -> Tuple[np.ndarray, Jacobians]:
        ...
        jacs = Jacobians()
        raw_error = self._error_func.evaluate(
            self._noise_model.get_sqrt_information(), jacs
        )
