...
class SlidingWindowFilter(Problem):
    def __init__(self):
        ...
        self._related_var_keys = (
            {}
        )  # dict of related variables (check if we can stop re-linearizing)
    ...
    def marginalize_variable(self, *variables):
        ...
        for variable in variables:
            assert (
                variable.key in self._variables.keys()
            ), "variable key not found in variables dictionary"
    ...
    def add_cost_term(self, *cost_terms):
        for cost_term in cost_terms:
            ...
            for key in related_var_keys:
                assert (
                    key in self._related_var_keys.keys()
                ), "related variable key not found in related variables dictionary"
