...
def test_direct_measure():
    ...
    loss_func = problem.L2LossFunc()
    noise_model = problem.StaticNoiseModel(np.eye(6), "covariance")
    error_func = LogMapEvaluator(
        ComposeEvaluator(meas_T_ba_var, InverseEvaluator(hat_T_ba_var))
    )
...
def test_relative_change():
    ...
    error_func1 = LogMapEvaluator(
        ComposeEvaluator(meas_T_10_var, InverseEvaluator(hat_T_10_var))
    )
    cost_term1 = problem.WeightedLeastSquareCostTerm(
        error_func1, noise_model, loss_func
    )
    # measured relative change
    error_func2 = LogMapEvaluator(
        ComposeEvaluator(
            ComposeEvaluator(meas_T_21_var, hat_T_10_var),
            InverseEvaluator(hat_T_20_var),
        )
    )
    cost_term2 = problem.WeightedLeastSquareCostTerm(
        error_func2, noise_model, loss_func
    )
